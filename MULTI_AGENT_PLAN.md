...
- **Tue Jul 14:** Eval Card before/after comparison — run the PRD's golden/edge-case/adversarial fixtures plus `presentation/demo/*` through both `run_agent_legacy` and the new split against the same seeded docs, diff per-category coverage, fix regressions. Go/no-go call on flipping the default.
- **Wed Jul 15:** Phase 2 concurrency — `ThreadPoolExecutor` dispatch, `docs_client.py` write lock, two independent `OpenRouterClient` instances, concurrency test.
- **Thu Jul 16 — buffer/polish, feature-complete ("more or less done"):** Full regression (`pytest`, `ruff`, `mypy`, `--cov-fail-under=90`), CHANGELOG/README updates, merge `multi-agent-plan` into `main`. Any Mon–Wed spillover gets absorbed here, not into Friday.
- **Fri Jul 17 — demo prep only:** No new feature work. Update `presentation/build_deck.py`, `PRESENTATION_PLAN.md`, and demo fixtures for the new architecture; rehearse.
- **Sat Jul 18 — demo.**

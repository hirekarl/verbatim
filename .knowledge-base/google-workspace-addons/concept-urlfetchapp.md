...
- **`UrlFetchApp.fetch()` has a hard 60-second timeout, and it is not configurable.** Not "roughly 30–60s, raisable at the project level" — there is no project-level or per-call override at all; Apps Script simply doesn't expose one, unlike most HTTP client libraries. Confirmed against Google's own docs and community reports: [Justin Poehnelt's UrlFetchApp reference](https://justin.poehnelt.com/posts/definitive-guide-to-urlfetchapp/); the Apps Script issue tracker has an open, unresolved feature request asking for exactly this configurability ([issue #36761852](https://issuetracker.google.com/issues/36761852)), which confirms it doesn't exist today. Verbatim's real audit latency (up to `max_tool_call_rounds=20` sequential LLM round trips) already exceeds this in practice — the synchronous request/response shape in the minimal example above cannot hold up for a real audit; the backend needs to return immediately with a job ID and have the Add-on poll a second endpoint instead.

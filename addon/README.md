...
1. `clasp push --force` to upload `appsscript.json`/`Code.gs`/`Backend.gs`. `--force` is needed the first time since `clasp create`'s own manifest differs from this repo's. `appsscript.json`'s `urlFetchWhitelist` must match whatever `BACKEND_URL` you set below — it's optional for **Deploy → Test deployments** but Google rejects a **versioned** deployment without it, so update both together if the backend URL ever changes (see `.knowledge-base/google-workspace-addons/concept-appsscript-manifest.md`).

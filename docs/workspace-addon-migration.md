...
| `agent.py`                                                          | No change — `run_agent()` takes an already-constructed `GoogleDocsClient`/`AnthropicClient` and doesn't care how either was authenticated.                                                                                                                                                                                                                                                                                                                       |
...
Recommend **Cloud Run** over Cloud Functions: this is already a normal Python app with real dependencies (`google-api-python-client`, `anthropic`, etc.), and the tool-calling loop can run up to `max_tool_call_rounds=20` LLM round trips, which wants more headroom over request timeout and concurrency than a single function gives. `ANTHROPIC_API_KEY` moves to Secret Manager (replacing the local `.env`); the container is built from the existing `src/verbatim` package; no persistent volume is needed since there's no more `token.json` to manage.
...
Still outstanding: setting the Add-on's Script Properties (`BACKEND_URL`, `BACKEND_SHARED_SECRET`, optional `DEFAULT_BRIEF_ID` — see `addon/README.md`; brief ID and target channel are sidebar inputs, not Script Properties) and an actual end-to-end run via **Deploy → Test deployments** against a real Google Doc. Both are manual, in-browser steps with no CLI/API path.
...
gcloud run deploy verbatim-backend \
  ...
  --set-secrets="ANTHROPIC_API_KEY=anthropic-api-key:latest,BACKEND_SHARED_SECRET=backend-shared-secret:latest" \
  --project=verbatim-501715
```

Notes on that command:

- `GOOGLE_OAUTH_CLIENT_ID` is a plain env var, not a Secret Manager secret — it's a public client identifier (not sensitive), unlike `ANTHROPIC_API_KEY`.
...
- `anthropic-api-key` and `backend-shared-secret` must already exist as Secret Manager secrets in the target project (`gcloud secrets create <name> --data-file=-`), and the Cloud Run service's runtime service account needs `roles/secretmanager.secretAccessor` on each.

### 6.1 Async job submission + polling (added 2026-07-09)

The single blocking `POST /audit` call above couldn't survive real audit latency: `UrlFetchApp.fetch()` on the Apps Script side has a hard, non-configurable 60-second timeout (see `.knowledge-base/google-workspace-addons/concept-urlfetchapp.md`), and a real audit (`max_tool_call_rounds=20`) routinely exceeds it. `POST /audit` now returns `202 {job_id}` immediately and runs `run_agent()` on a background thread; the Add-on polls `GET /audit/{job_id}` for the outcome. See `addon/README.md`'s Known limitation section and `src/verbatim/http_api.py`.

The job store is a plain in-process `dict`, only valid because the service is pinned to a single instance — a demo-scoped shortcut, same spirit as `--allow-unauthenticated` above; a real multi-user deployment would need Firestore/Redis/etc instead. This needs two additional flags beyond the original deploy command:

- `--min-instances=1 --max-instances=1` — pins one warm container so the in-process job dict is visible to every request, instead of scattered across Cloud Run's default scale-to-zero/scale-out behavior.
- `--no-cpu-throttling` — **required, not optional**. By default Cloud Run only allocates CPU to a container while it's actively handling a request; a background thread started during one request gets starved between polls and only inches forward once per subsequent request that happens to land on the same instance. Confirmed directly: without this flag, the audit only progressed when a "Check Status" poll came in, instead of running continuously in the background. See [Google Cloud's writeup on always-on CPU for background work](https://cloud.google.com/blog/topics/developers-practitioners/use-cloud-run-always-cpu-allocation-background-work). This means paying for the pinned instance's full uptime rather than just request-processing time — acceptable paired with `--min-instances=1`, worth reconsidering before ever removing that pin.

Applied via `gcloud run services update verbatim-backend --region=us-east4 --project=verbatim-501715 --min-instances=1 --max-instances=1 --no-cpu-throttling` (an incremental update after the original `gcloud run deploy`, not re-run with `--set-env-vars`/`--set-secrets`, which would risk clobbering the existing config).

## 7. Knowledge-base gap

`.knowledge-base/` currently has zero coverage of Apps Script, `CardService`, the Add-on manifest (`appsscript.json`), `UrlFetchApp`, or Workspace Add-on OAuth/publishing — unlike the Docs/Drive/Anthropic REST APIs already mapped there. Action item for whenever this work is actually picked up (not done as part of this doc): add `.knowledge-base/google-workspace-addons/MAP.md` plus leaves, following the existing map-and-leaf convention in `.knowledge-base/README.md`. Likely leaves: `concept-appsscript-manifest.md`, `concept-cardservice-ui.md`, `concept-oauth-scopes-and-triggers.md`, `concept-urlfetchapp.md`.

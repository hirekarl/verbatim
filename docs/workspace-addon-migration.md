...
- Exact inbound-token validation mechanism for the backend endpoint — still open, tracked on #21.
...
- ~~Whether v1 needs a brief-ID picker UI in the sidebar, or can start with a hardcoded/config value.~~ **Resolved** (see #24): start v1 with a hardcoded/config value, not a picker — keeps the Add-on shell a thin trigger per §3 and avoids extra Apps Script UI scope.

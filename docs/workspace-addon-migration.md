...
**Status: not scheduled — post-demo backlog. Do not start before the Sat Jul 11 demo.**

This isn't new scope. `docs/research-notes.snapshot.md` already states the product intent plainly: "To minimize workflow friction, the agent must be built as a **Google Docs Workspace Add-on**." The CLI built for the demo (`src/verbatim/cli.py`) is a stand-in for that target, not the final architecture — it just hasn't been written down anywhere until now.
...
| `cli.py`                                                            | Retired as the entrypoint; its argparse/print/exit-code logic ports into a small HTTP handler (e.g. a FastAPI/Flask route) that parses a JSON request body instead of argv and returns JSON instead of printing to stdout.                                                                       |
...
- Exact inbound-token validation mechanism for the backend endpoint.
- Single GCP project (Apps Script + backend together) vs. two.
- Ordering relative to Christina's already-planned post-demo rotation into Docs API/agent-loop territory (`list_comments` work) — does one block the other?
- Whether v1 needs a brief-ID picker UI in the sidebar, or can start with a hardcoded/config value.

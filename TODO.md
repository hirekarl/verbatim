...
- [x] [#23](https://github.com/hirekarl/verbatim/issues/23) Containerize backend and deploy to Cloud Run — depends on #20 (`Dockerfile` added; not build-tested — no Docker in this environment — and not actually deployed; see `docs/workspace-addon-migration.md` §6 for the remaining `gcloud` steps someone with GCP access needs to run)

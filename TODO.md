...
### Tue Jul 14 (Karl, out-of-band)

Two things landed today outside the Eval Card validation task above, in Karl-owned files only (no collision with Christina's Structural-agent work):

- [x] Migrate the LLM backend from OpenRouter to the native Anthropic SDK (`src/verbatim/llm_client.py`, new `AnthropicClient`). *(#73, merged, version-bumped to v0.15.0.)*
- [x] Diagnose and fix a live-doc audit failure: Cloud Run calls to the LLM backend were failing with an opaque "network error: Connection error." Root cause was a stray `\r\n` baked into the `anthropic-api-key` Secret Manager secret, not a network/VPC/IPv6 issue — traced only after adding logging of the raw underlying exception instead of swallowing it into `LLMClientError`. Also forced the Anthropic SDK's `httpx` client onto IPv4-only egress (Cloud Run has no outbound IPv6 route, and `api.anthropic.com` resolves to both an A and AAAA record) — a real latent risk, though not this incident's actual cause. *(`af7bc25`, version-bumped to v0.15.1.)*
...
- [x] ~~**Christina**: CHANGELOG/README updates for the new architecture.~~ *(Already satisfied without dedicated work: README already documents the multi-agent architecture and the Anthropic SDK setup, and CHANGELOG.md is auto-generated by `cz bump`. No gap to fill.)*
- [ ] **Christina** (proposed, pending Karl's confirmation): revisit the deferred [#69](https://github.com/hirekarl/verbatim/issues/69) golden-edge category-tiebreaker bug in `structural.py`, now that the CHANGELOG/README line above turned out to need no work and coverage moved to Wed Jul 15 (see above). It's real, substantive, in-domain work — not a new-territory rotation — and demo is still Jul 18, so there's room to take another pass at it if Christina wants to.
...
- [x] [#23](https://github.com/hirekarl/verbatim/issues/23) Containerize backend and deploy to Cloud Run — depends on #20. Deployed: `verbatim-backend` on `us-east4`, project `verbatim-501715`. The service has two permanent URLs (`gcloud run services describe`'s `run.googleapis.com/urls` annotation) — `https://verbatim-backend-75857425003.us-east4.run.app` (legacy format) and `https://verbatim-backend-4iub5rucnq-uk.a.run.app` (newer hash-based format, now what Cloud Run reports as `status.url`); both are whitelisted in `addon/appsscript.json`'s `urlFetchWhitelist` since either can end up in `BACKEND_URL`. `GOOGLE_OAUTH_CLIENT_ID` is now set (sourced from the associated Apps Script project's OAuth client).
- [ ] [#33](https://github.com/hirekarl/verbatim/issues/33) Front the Cloud Run backend with an internal auth-proxy to drop `--allow-unauthenticated` — deliberately deferred, not part of the initial deploy.
